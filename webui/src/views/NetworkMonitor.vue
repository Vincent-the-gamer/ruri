...
const rangeLabel = computed(() => {
  ...
  if (selectedRange.value === 7) return '7 天'
  return '1 天'
})
...
const chartColors = [
  '#5F7E9B', '#708865', '#9A7557', '#786696', '#5D8985',
  '#9C6674', '#80844F', '#69788D', '#A0526E', '#5A8A7A',
]
...
function createRequestChart() {
  ...
  const data = requestMetrics.value.series.map(p => ({ x: p.timestamp * 1000, y: p.value }))
  ...
  requestChart = new Chart(requestChartCanvas.value, {
    ...
    options: {
        ...
        tooltip: {
          callbacks: {
            title(items: TooltipItem<'line'>[]) {
              ...
              return x != null ? new Date(x as number).toLocaleString('zh-CN') : ''
            },
          }
        },
      },
      scales: {
        x: {
          type: 'time',
          time: { unit: selectedRange.value === 1 ? 'hour' : selectedRange.value === 3 ? 'hour' : 'day' },
          grid: { color: isDark ? '#ffffff15' : '#00000010' },
        },
        y: {
          beginAtZero: true,
          ticks: { callback: (v) => typeof v === 'number' ? formatNumber(v) : v },
          grid: { color: isDark ? '#ffffff15' : '#00000010' },
        },
      },
    },
...
function createTrafficChart() {
  ...
  const dataIn = trafficMetrics.value.series_in.map(p => ({ x: p.timestamp * 1000, y: p.value }))
  const dataOut = trafficMetrics.value.series_out.map(p => ({ x: p.timestamp * 1000, y: p.value }))
  const isDark = document.documentElement.classList.contains('dark')
  ...
  trafficChart = new Chart(trafficChartCanvas.value, {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            title: (items) => items[0]?.parsed?.x ? new Date(items[0].parsed.x).toLocaleString('zh-CN') : '',
            label: (item) => `${item.dataset.label}: ${formatBytes(item.parsed.y)}`,
          },
        },
      },
      scales: {
        ...
        y: {
          beginAtZero: true,
          ticks: { callback: (v) => typeof v === 'number' ? formatBytes(v) : v },
          grid: { color: isDark ? '#ffffff15' : '#00000010' },
        },
      },
    },
...
function createTokenChart() {
  ...
  const datasets = providers.map((p, i) => {
    const pointMap = new Map(p.points.map(pt => [pt.timestamp * 1000, pt.value]))
    return {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            title: (items) => items[0]?.parsed?.x ? new Date(items[0].parsed.x).toLocaleString('zh-CN') : '',
            label: (item) => `${item.dataset.label}: ${formatTokens(item.parsed.y)} tokens`,
          },
        },
      },
      scales: {
        ...
        y: {
          stacked: true,
          beginAtZero: true,
          ticks: { callback: (v) => typeof v === 'number' ? formatTokens(v) : v },
          grid: { color: isDark ? '#ffffff15' : '#00000010' },
        },
      },
    },
  ...
  <div class="monitor-container">
    ...
    <header class="monitor-header">
      <div>
        ...
        <p class="page-subtitle">API 请求数、网络流量和 Token 消耗统计</p>
      </div>
    ...
    <template v-else>
      ...
      <div class="overview-grid">
        <div class="overview-card">
          ...
          <div class="card-value">{{ formatNumber(requestMetrics?.total ?? 0) }}</div>
          <div class="card-note">过去 {{ rangeLabel }}</div>
        </div>
        <div class="overview-card">
          ...
          <div class="card-value">{{ formatBytes((trafficMetrics?.total_in ?? 0) + (trafficMetrics?.total_out ?? 0)) }}</div>
          <div class="card-note">接收 {{ formatBytes(trafficMetrics?.total_in ?? 0) }} / 发送 {{ formatBytes(trafficMetrics?.total_out ?? 0) }}</div>
        </div>
        <div class="overview-card">
          ...
          <div class="card-value">{{ formatTokens(tokenMetrics?.total_tokens ?? 0) }}</div>
          <div class="card-note">过去 {{ rangeLabel }}</div>
        </div>
        <div class="overview-card" v-if="tokenMetrics?.tokens_by_provider?.length">
          ...
          <div class="card-label">活跃提供商</div>
          <div class="card-value">{{ tokenMetrics.tokens_by_provider.length }}</div>
          <div class="card-note">{{ tokenMetrics.tokens_by_provider.map(p => p.provider_name).join(', ') }}</div>
        </div>
      </div>
      ...
      <div class="section-toolbar">
        <div>
          ...
          <p class="section-subtitle">按时间查看 API 请求数变化</p>
        </div>
      ...
      <div class="section-header">
        <h2 class="section-title">网络流量</h2>
        <p class="section-subtitle">按时间查看网络流量变化</p>
      </div>
      <div class="chart-card">
      ...
      <div class="section-header">
        ...
        <p class="section-subtitle">按提供商查看 Token 消耗分布</p>
      </div>
      <div class="chart-card">
      ...
      <!-- Provider Ranking -->
      <div v-if="tokenMetrics?.tokens_by_provider?.length" class="section-header">
        <h2 class="section-title">提供商排名</h2>
        <p class="section-subtitle">过去 {{ rangeLabel }} 的 Token 消耗排名</p>
      </div>
      <div v-if="tokenMetrics?.tokens_by_provider?.length" class="ranking-card">
        <div
          v-for="(p, i) in [...tokenMetrics.tokens_by_provider].sort((a, b) => b.tokens - a.tokens)"
          :key="p.provider_name"
          class="ranking-row"
        >
          ...
          <div class="ranking-bar-wrap">
            <div
              class="ranking-bar"
              :style="{
                width: tokenMetrics.tokens_by_provider.length > 0 && p.tokens > 0
                  ? (p.tokens / Math.max(...tokenMetrics.tokens_by_provider.map(x => x.tokens)) * 100) + '%'
                  : '0%',
                backgroundColor: chartColors[i % chartColors.length],
              }"
            />
          </div>
          <span class="ranking-value">{{ formatTokens(p.tokens) }}</span>
        </div>
      </div>
    </template>
  </div>
...
.card-icon.requests { background: #eef2ff; color: #5F7E9B; }
.card-icon.traffic { background: #ecfdf5; color: #708865; }
.card-icon.tokens { background: #fef9e7; color: #9A7557; }
.card-icon.providers { background: #f3e8ff; color: #786696; }
