                ...
                event_result = stream.next() => {
                    match event_result {
                        Some(Ok(event)) => {
                            ...
                            if let crate::types::StreamEvent::Done { .. } = &event {
                                ...
                                if segmented_reply_enabled && !full_content.is_empty() && accumulated_tool_calls.is_empty() {
                                    ...
                                    for (i, segment) in segments.into_iter().enumerate() {
                                        let seg_event = crate::types::StreamEvent::SegmentedContentDelta {
                                            segment_index: i,
                            ...
                            // When segmented reply is enabled, skip forwarding regular ContentDelta
                            // events during streaming — we send the segmented version at Done instead.
                            // All other events (tool_executing, tool_call_*, etc.) are still forwarded.
                            if segmented_reply_enabled && matches!(&event, crate::types::StreamEvent::ContentDelta { .. }) {
                                continue;
                            }
