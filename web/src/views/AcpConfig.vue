  ...
  <div class="page">
    ...
    <div class="page-header">
      <div class="header-info">
        <h1 class="header-title">ACP 模式配置</h1>
        <p class="header-desc">
          配置 Agent Client Protocol 模式使用的模型供应商和技能
        </p>
      </div>
    </div>
    ...
    <!-- Loading -->
    <div
      v-if="acpStore.loading && !acpStore.config"
      class="loading-state"
    >
      加载中...
    </div>
    ...
    <template v-else-if="acpStore.config">
      ...
      <section class="config-section">
        ...
        <p class="section-desc">选择 ACP 模式使用的模型供应商</p>

        <div
          v-if="acpStore.config.available_providers.length === 0"
          class="empty-section"
        >
          <p class="empty-section-text">暂无可用供应商</p>
          ...
          >
            ...
            <div class="provider-info">
              ...
              <div class="provider-meta">
                ...
                <span class="provider-model">{{ provider.default_model }}</span>
              </div>
            </div>
          </button>
        </div>
      ...
      <section class="config-section">
        ...
        <p class="section-desc">选择 ACP 模式启用的技能</p>

        <div
          v-if="acpStore.config.available_skills.length === 0"
          class="empty-section"
        >
          <p class="empty-section-text">暂无可用技能</p>
          <p class="empty-section-hint">
            请先在「技能」页面添加技能
          </p>
        </div>
        ...
        <div v-else class="skill-list">
          <div
            ...
            class="skill-option"
            :class="{
              'skill-option--selected': isSkillSelected(skill.name),
            }"
          >
            ...
            <button
              class="toggle"
              :class="{ 'toggle--on': isSkillSelected(skill.name) }"
      ...
      <div class="save-row">
        ...
        <span v-if="hasChanges" class="change-hint">有未保存的更改</span>
      </div>
      ...
      <div class="info-banner">
        ...
        <span>
          ACP 模式在独立进程中运行，通过 stdio 与客户端通信。此处配置的供应商和技能将在 ACP 模式启动时生效。
        </span>
      </div>
    </template>
  </div>
